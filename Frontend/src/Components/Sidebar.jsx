...
import { LayoutDashboard, FolderEdit, Users, Settings, LogOut } from "lucide-react";
...
export default function Sidebar() {
  ...
  const linkStyle =
    "flex items-center gap-3 px-4 py-3 rounded-xl text-gray-500 hover:bg-white/40 hover:text-[#111827] transition whitespace-nowrap w-full text-left";
  const activeStyle =
    "bg-gradient-to-r from-blue-900 to-cyan-300 text-white shadow";
  const iconStyle = "w-5 h-5 min-w-[20px]";
  const settingsActive = location.pathname.startsWith("/settings");
  const logoutActive = location.pathname === "/login";
    ...
    <aside className="w-60 shrink-0 h-fit bg-[#cfd3d7] text-gray-700 backdrop-blur-md border-r border-white/50 p-4 shadow-gray-700 shadow-2xl rounded-3xl m-5">
      ...
      <nav className="flex flex-col gap-2">
        ...
        {isManager && (
          <>
            <NavLink to="/addproject"
              className={({ isActive }) => `${linkStyle} ${isActive ? activeStyle : ""}`}>
              <FolderEdit className={iconStyle} />
              {t("addEditProject")}
            </NavLink>
        ...
        <NavLink to="/settings/profile"
          className={`${linkStyle} ${settingsActive ? activeStyle : ""}`}>
          <Settings className={iconStyle} />
          {t("settings")}
        </NavLink>

        {/* ✅ Logout — same style as other nav items, activates when on /login */}
        <button
          onClick={handleLogout}
          className={`${linkStyle} ${logoutActive ? activeStyle : ""}`}
        >
